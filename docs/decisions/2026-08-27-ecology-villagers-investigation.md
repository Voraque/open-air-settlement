...
| Forest renewal | Green Cuts is the leading narrow replacement, but its official 1.0.4 Fabric jar is currently rejected pending a packaging repair. | It only tracks dropped saplings and checks that the target block is survivable and unoccupied. Its published jar embeds Configurate-HOCON but omits Configurate Core and its runtime graph; a headless full-pack test fails before server startup with `NoClassDefFoundError`. A private packaging-only repair is being tested. |
...
The headless harness deliberately stages a fresh copy and excludes worlds, logs, backups, caches, Java runtimes, and credential-like files. Its own Pester suite currently passes 6/6 tests. The baseline pack reaches `Done` in roughly 136 seconds in the disposable lab, but is not yet considered clean: it has existing recipe parse failures, and an automatic post-start datapack reload delayed shutdown beyond the first harness timeout. Both are now explicit test failures instead of hidden startup noise.
