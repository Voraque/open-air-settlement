...
The checked-in server settings are `server-config/cropxp.json`, `server-config/celestria.json`, `server-config/worn_path.json5`, and `server-config/enchantinginfuser-server.toml`. They are server-only, so neither player needs a client-side mod for these changes.
