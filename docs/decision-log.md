...
Costs ship at the mod's defaults. The generated config, now checked in at `server-config/enchantinginfuser-server.toml`, has `[normal_infuser]` and `[advanced_infuser]` sections carrying `allow_repairing`, `allow_modifying_enchantments`, `maximum_bookshelves`, `allow_anvil_enchantments`, `allow_books` and `increase_anvil_repair_cost`, each with a nested `repair` and `costs` table. Every value in the checked-in file equals the default the file itself documents. The tier split is real: the basic infuser is `allow_repairing = "NOTHING"`, `allow_modifying_enchantments = "UNENCHANTED"`, no books and no anvil-only enchantments, while the advanced one repairs tools and armor, modifies anything, and allows both. `maximum_bookshelves` is 15 on both tiers. That last one matters here: the mod's own description says it exists because "enchanting costs will become ludicrously cheap with many modded enchantments present," and this pack has a very large modded enchantment load, so leaving it at the default `true` is the correct starting point rather than an untested one.
...
Server verification (2026-09-05): the FadeHost server was Online and empty (`list` returned 0 players; the only recent session, benjaminkamote, had disconnected at 23:25:59), so the jar and the datapack were uploaded over SFTP and the server restarted rather than waiting for a join. The pre-change listing is on the server root as `mods-pre-infuser-20260905-1633.txt`; `mods/` went from 116 to 117 jars. The 23:35 boot log shows `Loading 227 mods` including `- enchantinginfuser 21.1.4`, reaches `Done (1.366s)`, and contains no error or exception mentioning the mod. No new file in `crash-reports/`; the newest is still `crash-2026-09-04_10.37.00-server.txt`.

`datapack list enabled` names `[file/open-air-infuser-tags]` alongside `[file/skill-tree-tuning]`, which settles assumption A1 from the plan: Moonlight global datapacks are read on the dedicated server, not just the client, so the Mending tag did not need to move to `world/datapacks/`.

Deployment order differed from the plan's D7. Phase 2 was skipped, so no instance had generated a config to upload ahead of first boot. Rather than hand-write the file, the server was allowed to generate it and the result was pulled back over SFTP and checked in. Since D3 ships defaults unchanged, the generated defaults are the intended values, and the checked-in file is now the real one rather than a reconstruction.

Still not verified: opening either block. No modded item has been put into an infuser on any pack. The boot checks prove the mod loads, registers, and does not crash the server; they say nothing about the menu, which is where this mod's reported failures on other packs occur.
